...
bool AudioGeneratorWAV::loop()
{
  ...
  do
  {
    if (bitsPerSample == 8) {
      ...
      if(!GetBufferedData(3, (uint8_t*)lastSample+1)) stop();
    } else if (bitsPerSample == 32) {
      if(!GetBufferedData(4, lastSample)) stop();
    }
...
bool AudioGeneratorWAV::ReadWAVInfo()
{
  ...
  // Header == "RIFF"
  if (!ReadU32(&u32)) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data\n"));
    return false;
  };
  if (u32 != 0x46464952) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, invalid RIFF header, got: %08X \n"), (uint32_t) u32);
    return false;
  }
  ...
  if (u32 != 0x45564157) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, invalid WAVE header, got: %08X \n"), (uint32_t) u32);
    return false;
  }
  ...
  if (u32 == 16) { toSkip = 0; }
    ...
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, appears not to be standard PCM \n"));
    return false;
  ...
  if (u16 != 1) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, AudioFormat appears not to be standard PCM \n"));
    // return false;
  } // we only do standard PCM
  ...
  if ((channels<1) || (channels>2)) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, only mono and stereo are supported \n"));
    return false;
  ...
  if (sampleRate < 1) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, unknown sample rate \n"));
    return false;
  ...
  if ((bitsPerSample!=8) && (bitsPerSample != 16) && (bitsPerSample != 24) && (bitsPerSample != 32)) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, only 8 or 16 bits is supported \n"));
    return false;
  }  // Only 8 or 16 bits
  ...
  do {
    ...
    if(!file->seek(u32, SEEK_CUR)) {
      printf(("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data, seek failed\n"));
      return false;
    }
  } while (1);
  if (!file->isOpen()) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, file is not open\n"));
    return false;
  };
  ...
  if (!buff) {
    printf(("AudioGeneratorWAV::ReadWAVInfo: cannot read WAV, failed to set up buffer \n"));
    return false;
  };
  buffPtr = 0;
  buffLen = 0;
...
{
  if (!source) {
    printf(("AudioGeneratorWAV::begin: failed: invalid source\n"));
    return false;
  }
  file = source;
  if (!output) {
    printf(("AudioGeneratorWAV::begin: invalid output\n"));
    return false;
  }
  this->output = output;
  if (!file->isOpen()) {
    printf(("AudioGeneratorWAV::begin: file not open\n"));
    return false;
  } // Error

  if (!ReadWAVInfo()) {
    printf(("AudioGeneratorWAV::begin: failed during ReadWAVInfo\n"));
    return false;
  }

  if (!output->SetRate( sampleRate )) {
    printf(("AudioGeneratorWAV::begin: failed to SetRate in output\n"));
    return false;
  }
  if (!output->SetBitsPerSample( bitsPerSample )) {
    printf(("AudioGeneratorWAV::begin: failed to SetBitsPerSample in output\n"));
    return false;
  }
  if (!output->SetChannels( channels )) {
    printf(("AudioGeneratorWAV::begin: failed to SetChannels in output\n"));
    return false;
  }
  if (!output->begin()) {
    printf(("AudioGeneratorWAV::begin: output's begin did not return true\n"));
    return false;
  }
